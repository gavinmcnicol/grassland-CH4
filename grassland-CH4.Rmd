...
monthlyWTD <- calc_monthlyWTD(df_USsne) %>% 
  full_join(calc_monthlyWTD(df_USsnd)) %>% 
  full_join(calc_monthlyWTD(df_SEdeg))

monthly <- monthly %>% 
  full_join(monthlyWTD, by = 'site')

df_WTD <- calc_annualWTD(df_USsne) %>% 
  full_join(calc_annualWTD(df_USsnd)) %>% 
  full_join(calc_annualWTD(df_SEdeg))

annual <- annual %>% 
  full_join(annualWTD, by = c('site', 'year'))

annualWTD <- annual %>% 
  inner_join(df_WTD, by=c('year', 'site'))
