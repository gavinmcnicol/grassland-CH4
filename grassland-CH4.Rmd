...
```{r packages and functions, message=FALSE, warning=FALSE, include=FALSE, return=FALSE}
library(tidyverse)
library(lubridate)
# library(amerifluxr)
library(patchwork)
...
calc_annual <- function(df) {
  ...
  summarize(FCH4_an = sum(FCH4_F, na.rm = TRUE) * 1800 * 10^-9 * 16.043,
...
calc_monthly <- function(df) {
  ...
  summarize(FCH4_mon = sum(FCH4_F, na.rm = TRUE) * 1800 * 10^-9 * 16.043,
